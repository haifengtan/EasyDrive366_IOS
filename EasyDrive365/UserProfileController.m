...
//  EasyDrive366
//
//  Created by Steven Fu on 4/1/14.
...
@interface UserProfileController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>{
    id _json;
...
-(void)load_data{
    NSString *url = [NSString stringWithFormat:@"upload/init_user_docpic?userid=%d",[AppSettings sharedSettings].userid];
...
-(void)tapOnSmallImage:(UITapGestureRecognizer *)tap{
    ...
    NSString *url = [NSString stringWithFormat:@"upload/get_user_docpic?userid=%d&typeid=%@",[AppSettings sharedSettings].userid,item[@"id"]];
    [[AppSettings sharedSettings].http get:url block:^(id json) {
        if ([[AppSettings sharedSettings] isSuccess:json]){
...
{
    NSString *url =[NSString stringWithFormat:@"upload/upload_user_docpic?userid=%d&typeid=%@",[AppSettings sharedSettings].userid,_current[@"id"]];
